...
export default function GlobalListsPage() {
  const [lists, setLists] = useState<GlobalList[]>(() => store.getGlobalLists());
  ...
  const handleCsvImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      // Simple CSV parsing (split by comma or newline), deduplicate
      const imported = text.split(/[\n,]/)
        .map(item => item.trim())
        .filter(item => item.length > 0 && item !== 'name' && item !== 'value');
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (editingList) {
      store.updateGlobalList(editingList.id, { name, items: sortedItems, sorting, isCaseSensitive });
    } else {
      store.addGlobalList(name, sortedItems, sorting, isCaseSensitive);
    }
                  ...
                  {confirmDeleteId === list.id ? (
                    <div className="flex items-center gap-1">
                      <button 
                        ...
                        className="px-2 py-1 bg-red-500 text-white text-[9px] font-bold rounded hover:bg-red-600"
                      >Delete?</button>
                      <button 
                        onClick={() => setConfirmDeleteId(null)}
                        className="p-1 text-[#141414]/30 hover:text-[#141414]"
                      ><X size={14} /></button>
                    </div>
                  ) : (
                    <button 
                ...
                <form onSubmit={handleSave} className="space-y-8">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <select 
                        ...
                        onChange={(e) => {
                          ...
                          setSorting(val);
                          setItems(applySorting(items, val, isCaseSensitive));
                        }}
                      ...
                      <div className={`w-5 h-5 flex items-center justify-center border-2 transition-all ${isCaseSensitive ? 'bg-[#009688] border-[#009688]' : 'border-[#141414]/20'}`}>
                        ...
                        <input 
                          ...
                          onChange={(e) => {
                            ...
                            setIsCaseSensitive(val);
                            setItems(applySorting(items, sorting, val));
                          }}
                        />
                      </div>
                  ...
                  <div className="space-y-4 pt-4 border-t border-[#141414]/5">
                        ...
                        <div key={idx} className="flex items-center gap-2 group">
                          <div className="flex-1">
                            <input 
                              type="text" 
                              value={item} 
                              onChange={(e) => updateItemInList(idx, e.target.value)}
                              className="w-full p-2.5 bg-white border border-[#141414]/10 rounded-md focus:outline-none focus:border-[#F27D26] text-sm"
                            />
                          </div>
                    ...
                    <div className="flex gap-2">
                      {duplicateWarning && (
                        <div className="w-full mb-2 px-3 py-2 bg-red-50 border border-red-200 rounded-md text-xs font-semibold text-red-600 flex items-center gap-2">
                          ⚠️ {duplicateWarning}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-3 pt-6 pb-8">
                    ...
                    <button type="submit" className="flex-1 bg-[#141414] text-white py-3 rounded-full text-sm font-bold hover:bg-[#F27D26] transition-all shadow-lg shadow-[#141414]/10">
