...
export default function DashboardPage() {
  ...
  const stats = [
    { label: 'Active Forms', value: '12', icon: FileText, color: 'text-blue-600' },
  ...
  const adminStats = [
    { label: 'Total Staff', value: '240', icon: Users, color: 'text-indigo-600' },
    { label: 'Departments', value: '8', icon: Building, color: 'text-purple-600' },
    { label: 'Completion Rate', value: '84%', icon: TrendingUp, color: 'text-emerald-600' },
  ];
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {user.role === UserRole.STAFF && (
          <Link 
            href="/dashboard/staff/new-assessment"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {(user.role === UserRole.SUPER_ADMIN || user.role === UserRole.ADMIN ? [...stats, ...adminStats.slice(0, 1)] : stats).map((stat, i) => (
          <motion.div
            key={stat.label}
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: i * 0.1 }}
            className="bg-white border border-[#141414]/10 p-6 shadow-sm hover:shadow-md transition-shadow"
          >
        ...
        <div className="space-y-6">
          <div className="bg-[#141414] text-white p-8">
            ...
            <p className="text-sm leading-relaxed mb-6 italic serif opacity-80">
              &quot;The new Medical Staff protocols have been updated. Please ensure all relevant assessments are completed by the end of the month.&quot;
            </p>
